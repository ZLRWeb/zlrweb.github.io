...
這個設計是針對一個**任務調度系統 (Job Scheduling System)**。在系統設計時，需要在不同方向進行權衡取捨。
...
- **任務類型 (Task Type)**：任務可以是短暫執行的腳本 (script) 或長時間運行的服務 (long-running service)，例如監聽 web 或一直運行。設計應包含這兩種類型 (job can be all type)。
...
- **Client**：客戶端，可以提交任務請求.
- **Submission**：接收客戶端的任務請求.
- **Data Store**：數據存儲，用於保存任務資訊和狀態。Submission 將任務保存到 Data Store。Dashboard 也從 Data Store 讀取任務狀態.
- **Queue (MQ)**：消息隊列，用於緩衝和解耦提交和執行過程。任務提交後進入 Queue。Worker 從 Queue 獲取任務.
...
- 對於寫入 QPS (提交任務)，估算結果也很低 (峰值約 50)。
- 考慮單數據庫的風險 (單點故障)。可以考慮使用主從複製 (Master-Replica) 或雙主結構 (Dual Master) 來提高可靠性，實現雙機熱備。
- 討論了使用 **NoSQL 數據庫的可能性**。來源資料中認為場景沒有強烈要求使用關係型數據庫 (relational database)。判斷使用 SQL 或 NoSQL 通常看三個信號：
  1.  **數據模型 (Data model)**：是否需要外鍵約束 (foreign key)。
  2.  **查詢模式 (Query pattern)**：是否需要做聯表查詢 (join)。
  3.  **一致性要求 (Consistency requirement)**：是否可以放寬到最終一致性 (eventual consistency) 而非強一致性 (strong consistency)。
...
- 考慮 MQ 的實現類型：**內存隊列 (in-memory queue)** 或 **持久化隊列 (persistent queue)**。
  - **內存隊列**：無狀態 (stateless)，擴展方便。缺點是數據可能丟失。
  - **持久化隊列**：數據不丟失。缺點是擴展較麻煩。
- 討論了同時使用 Data Store 和 MQ 可能帶來的問題：
  - 數據可能在兩邊重複存儲。
  - 可能導致兩邊數據不一致。
...
- **能否使用 MQ 作為 Data Storage (比如 Configuration Database)**？
  ...
  - 優勢：延遲非常低 (low latency)，能提供較低的 SLA 保證。
  - 缺點：增加工程師負擔。流表一體 (streaming database) 概念相對較新，接受度低。後期運維複雜性可能高 (Operation complexity)。
- **能否使用 Data Storage 作為 MQ (比如 Database as MQ)**？
  - 技術上可行，且業界接受度更高。例如 Google 的 Spanner 實現了基於其數據存儲的 Pull 和 Push 語義的隊列。業界有不少實例。
  - 缺點：單機性能可能不高 (除非使用 Spanner 這類高性能數據庫)。
  - 優勢：可以通過分庫分表 (sharding) 解決性能問題。在此低 QPS 場景下完全可行。
...
1.  **Pull 模型 (Worker 主動拉取)**：Worker 發起 RPC 請求，從 Informer 或 Queue 拉取任務。
    ...
    - 缺點：Worker 需要不停輪詢 (polling)，95% 時間可能空轉。RPC 從 Worker 發起，Worker 需要較高權限，安全性較差。Worker 可能崩潰，導致狀態無法更新。
2.  **Push 模型 (Informer 推送)**：Informer 發起 RPC，將任務推給 Worker. Informer 需要跟蹤 Worker 狀態.
    - 優點：解耦 Worker，只有在有任務時才啟動 Worker。
    - 缺點：Informer 需要長期追蹤 Worker 狀態。可能需要額外強一致性服務。Informer 需要知道每個 Task 對應的 Worker 地址.
    - Worker 需要定期發送心跳 (heartbeat) 更新狀態，否則可能擁塞。對 Long-running service 任務類型，Timeout 不可行。
3.  **混合模型 (Push + Sidecar)**：Informer 將 Task 推送出去。Worker 旁邊搭載一個 Sidecar 進程，負責監控 Worker 運行狀態並定期發送心跳 (每 60 秒一次)。
...
- **節約/優化資源**：
  - **Worker 隔離/沙箱 (Sandboxing)**：通常使用 VM 或容器隔離任務執行環境以保證安全性。容器 (Container) 更輕量、節約資源，但暴露更多攻擊面，需要額外隔離層如 GVisor。輕量級 VM (Lightweight VM) 如 Firecracker、Google 的 gVM 或 AWS Firecracker 更智能，體積小 (不超過 5MB)，啟動快 (小於 500ms)，同時保證安全性。選擇取決於使用場景和團隊工程能力.
  - **調度優化 (Scheduling Optimization)**：從 Worker 隔離 (可能浪費資源) 轉向混合部署 (Hyper-deployment)。在同一台機器上混合部署不同類型的任務 (如 IO 密集型和 CPU 密集型)，提高機器利用率。這需要開發特殊的調度算法。這被稱為重新分配資源 (reallocate resource)。
...
- **實現方式權衡**：可以將優先級隊列和拓撲排序邏輯放在 Informer 的內存中，避免維護額外服務，但增加了單個組件的複雜性。或者將它們抽成獨立的微服務。獨立微服務降低了複雜性，但增加了額外開銷。優點是增加了系統的可擴展性。最終方案取決於具體使用場景。

這個設計過程展示了從需求分析、功能定義、模型設計、架構草圖到細節討論的系統設計思路，並在過程中權衡不同方案的優劣及風險。特別是在數據存儲和隊列選擇上，應用了延遲決策原則。在 Worker 通信和資源管理方面，探討了多種可行的技術方案及其取捨。
